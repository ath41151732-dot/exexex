...
async function loadOnce(u){
  ...
  if(snap.exists()){
    ...
    if(remoteJSON!==lastSavedJson){
      ...
      lastSavedJson=remoteJSON; console.log("[sync-poll] applied remote update",events.length);
    }
  } else {
    await setDoc(doc(db,"users",u.uid),{email:u.email,events:[],createdAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:true});
    const local=getLocalEvents(); if(local.length) await cloudSave("initialUpload");
...
function hookSave(){
  ...
  if(typeof orig==="function" && !orig.__cloudHooked){
    ...
    wrapped.__cloudHooked=true; window.saveUserEvents=wrapped; console.log("[sync-poll] saveUserEvents hooked");
  }
}

function startPolling(u){
  if(pollTimer) clearInterval(pollTimer);
  pollTimer=setInterval(()=>loadOnce(u).catch(()=>{}), 5000); // 5초마다 확인
